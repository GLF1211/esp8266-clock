...
void TFT_eFEX::drawBmp(String filename , int16_t x , int16_t y , TFT_eSprite* _spr) {
    ...
    // Note: ESP32 passes "open" test even if fs::File does not exist, whereas ESP8266 returns NULL
    if (!SPIFFS.exists(filename)) {
        Serial.println(F(" fs::File not found")); // Can comment out if not needed
        return;
    }

    // Open requested fs::File
    bmpFS = SPIFFS.open(filename , "r");

    if (!bmpFS) {
        Serial.print("Bitmap fs::File not found");
        return;
    }
...
// These read 16 and 32-bit types from the fs::File.
...
void TFT_eFEX::drawJpeg(String filename , int16_t xpos , int16_t ypos , TFT_eSprite* _spr) {
    ...
    // Note: ESP32 passes "open" test even if fs::File does not exist, whereas ESP8266 returns NULL
    if (!SPIFFS.exists(filename)) {
        Serial.println(F(" Jpeg fs::File not found")); // Can comment out if not needed
        return;
    }

    // Open the named fs::File (the Jpeg decoder library will close it after rendering image)
    fs::File jpegFile = SPIFFS.open(filename , "r");    // fs::File handle reference for SPIFFS
    //  fs::File jpegFile = SD.open( filename, FILE_READ);  // or, fs::File handle reference for SD library

    // Use one of the three following methods to initialise the decoder:
    //boolean decoded = JpegDec.decodeFsFile(jpegFile); // Pass a SPIFFS fs::File handle to the decoder,
    //boolean decoded = JpegDec.decodeSdFile(jpegFile); // or pass the SD fs::File handle to the decoder,
    ...
    if (decoded) {
        ...
        Serial.println("Jpeg fs::File format not supported!");
    }
}
...
void TFT_eFEX::jpegInfo(String filename) {
    ...
    // Open the named fs::File (the Jpeg decoder library abort will close it after reading image info)
    fs::File jpegFile = SPIFFS.open(filename , "r");    // fs::File handle reference for SPIFFS
    //  fs::File jpegFile = SD.open( filename, FILE_READ);  // or, fs::File handle reference for SD library
    ...
    if (decoded) {
        ...
        Serial.println(FPSTR(line));
        Serial.println("");
        JpegDec.abort();    // Close fs::File and tidy up
    }
}
...
void TFT_eFEX::listSPIFFS(void) {
    ...
    static const char line[] PROGMEM = "=================================================";
    Serial.println(FPSTR(line));
    Serial.println(F("  fs::File name                              Size"));
    Serial.println(FPSTR(line));
...
void TFT_eFEX::listSPIFFS(void) {
    ...
    while (fs::File) {
        ...
        if (fs::File.isDirectory()) {
            ...
            String fileName = fs::File.name();
            Serial.print("  " + fileName);
            // fs::File path can be 31 characters maximum in SPIFFS
            int spaces = 33 - fileName.length(); // Tabulate nicely
            if (spaces < 1) spaces = 1;
            while (spaces--) Serial.print(" ");
...
bool TFT_eFEX::screenServer(void) {
    ...
    // where # is a number 0-9 and xxx is a fs::File type specified below
...
void TFT_eFEX::sendParameters(String filename) {
    ...
    Serial.write(*FILE_TYPE); // First character defines fs::File type j,b,p,t
}
...
// Include these in header fs::File
// #include "rom/tjpgd.h"
// #include "FS.h"
...
/**************************************************************************/
/*!
    @brief  Decode an array stored as a fs::File (ESP32 only)
    @param    Filing system, e.g. SPIFFS
    @param    fs::File name, precede with / for SPIFFS
    @param    Display x coord to draw at
    @param    Display y coord to draw at
    @param    Optional: Unscaled jpeg maximum width in pixels
    @param    Optional: Unscaled jpeg maximum height in pixels
...
bool TFT_eFEX::drawJpgFile(fs::FS& fs , const char* path , uint16_t x , uint16_t y , uint16_t maxWidth , uint16_t maxHeight , uint16_t offX , uint16_t offY , jpeg_div_t scale) {
    ...
    fs::File fs::File = fs.open(path , "r");
    if (!fs::File) {
        log_e("Failed to open fs::File for reading");
        return false;
    }
...
static uint32_t jpgReadFile(JDEC* decoder , uint8_t* buf , uint32_t len) {
    ...
    fs::File* fs::File = (fs::File*) jpeg->src;
    if (buf) {
        return fs::File->read(buf , len);
    } else {
        fs::File->seek(len , fs::SeekCur);
    }
    return len;
}
