...
// fs::File names must be alpha-numeric characters (0-9, a-z, A-Z) or "/" underscore "_"
...
public:
    ...
    // List information about a Jpeg fs::File to the Serial port (uses JPEGDecoder library)
    ...
    // Screen server call with or without a filename for PC stored image fs::File
    ...
    // Draw a jpeg stored in a fs::File using the ESP32 native decoder, can crop and scale (Tested with SPIFFS fs::File)
