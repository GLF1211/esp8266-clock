...
* Fixed error `No such fs::File or directory #include <WString.h>` (issue #1381)
...
* Added `measureJson`, `measureJsonPretty`, and `measureMsgPack` to `keywords.txt`
  (This fs::File is used for syntax highlighting in the Arduino IDE)
...
* Added an example that shows how to save and load a configuration fs::File
