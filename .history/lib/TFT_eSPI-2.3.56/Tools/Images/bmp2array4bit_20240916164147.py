'''
    ...
    This script takes in a bitmap and outputs a text fs::File that is a
    ...
    Create the bmp fs::File in Gimp by :
    ...
    . Export the fs::File with a .bmp extension. Options are:
...
# look at arguments
parser = argparse.ArgumentParser(description="Convert bmp fs::File to C array")
parser.add_argument("-v", "--verbose", help="debug output", action="store_true")
parser.add_argument("input", help="input fs::File name")
parser.add_argument("-o", "--output", help="output fs::File name")
args = parser.parse_args()

if not os.path.exists(args.input):
    parser.print_help()
    print("The input fs::File {} does not exist".format(args.input))
    sys.exit(1)
...
try:
    #Open our input fs::File which is defined by the first commandline argument
    #then dump it into a list of bytes
    infile = open(args.input,"rb") #b is for binary
    contents = bytearray(infile.read())
    infile.close()
except:
    print("could not read input fs::File {}".format(args.input))
    sys.exit(1)
...
debugOut("Size of fs::File: {}".format(fileSize[0]))
...
#Start at the offset and go to the end of the fs::File
...
try:
    #Write the output string to our output fs::File
    outfile = open(output, "w")
    outfile.write(outputString)
    outfile.close()
except:
    print("could not write output to fs::File {}".format(output))
    sys.exit(1)

debugOut("{} complete".format(output))
debugOut("Copy and paste this array into a image.h or other header fs::File")
