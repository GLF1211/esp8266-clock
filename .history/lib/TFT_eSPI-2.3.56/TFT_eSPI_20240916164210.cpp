...
TFT_eSPI::TFT_eSPI(int16_t w , int16_t h) {
    ...
    _init_width = _width = w; // Set by specific xxxxx_Defines.h fs::File or by users sketch
    _init_height = _height = h; // Set by specific xxxxx_Defines.h fs::File or by users sketch
    ...
    } else // Must be 1bpp
    {
        ...
        for (int32_t yp = dy; yp < dy + dh; yp++) {
            ...
            for (int32_t xp = dx; xp < dx + dw; xp++) {
                ...
                if (col) { *linePtr++ = bitmap_fg >> 8; *linePtr++ = (uint8_t) bitmap_fg; } else { *linePtr++ = bitmap_bg >> 8; *linePtr++ = (uint8_t) bitmap_bg; }
            }
            data += ww;
            pushPixels(lineBuf , dw);
        }
        ...
        if ((size == 1) && fillbg && !clip) {
            ...
            for (int8_t j = 0; j < 8; j++) {
                for (int8_t k = 0; k < 5; k++) {
                    if (column [k] & mask) { tft_Write_16(color); } else { tft_Write_16(bg); }
                }
                mask <<= 1;
                tft_Write_16(bg);
            }
        ...
        if (textcolor == textbgcolor || textsize != 1 || clip) {
            ...
            for (int32_t i = 0; i < height; i++) {
                ...
                for (int32_t k = 0; k < w; k++) {
                    ...
                    mask = 0x80;
                    while (mask && pX) {
                        if (line & mask) { tft_Write_16(textcolor); } else { tft_Write_16(textbgcolor); }
                        pX--;
                        mask = mask >> 1;
                    }
                }
        ...
        if (textcolor == textbgcolor && !clip) {
            ...
            if (textcolor != textbgcolor && textsize == 1 && !clip) {
                ...
                while (pc < w) {
                    line = pgm_read_byte((uint8_t*) flash_address);
                    flash_address++;
                    if (line & 0x80) { pcol = textcolor; line &= 0x7F; pf = true; } else { pcol = textbgcolor; if (textcolor == textbgcolor) pf = false; }
                    line++;
                    px = pc % width;
                    tx = x + textsize * px;
                    py = pc / width;
...
void TFT_eSPI::getSetup(setup_t& tft_settings) {
    // tft_settings.version is set in header fs::File
