...
/***************************************************************************************
** Function name:           loadFont
** Description:             loads parameters from a font vlw fs::File
*************************************************************************************x*/
...
void TFT_eSPI::loadFont(String fontName , bool flash) {
    /*
      ...
      Header of vlw fs::File comprises 6 uint32_t parameters (24 bytes total):
      ...
      The bitmaps start next at 24 + (28 * gCount) bytes from the start of the fs::File.
    ...
    */
    ...
    if (fs_font) {
        ...
        // Avoid a crash on the ESP32 if the fs::File does not exist
        if (fontFS.exists("/" + fontName + ".vlw") == false) {
            Serial.println("Font fs::File " + fontName + " not found!");
            return;
        }
    ...
    gFont.gCount = (uint16_t) readInt32(); // glyph count in fs::File
...
void TFT_eSPI::loadMetrics(void) {
    ...
    if (psramFound()) {
        ...
        gBitmap = (uint32_t*) ps_malloc(gFont.gCount * 4); // seek pointer to glyph bitmap in the fs::File
    } else
#endif
    {
        ...
        gBitmap = (uint32_t*) malloc(gFont.gCount * 4); // seek pointer to glyph bitmap in the fs::File
    }
...
** Description:             Get a 32 bit integer from the font fs::File
*************************************************************************************x*/
...
** Description:             Get the font fs::File index of a Unicode character
*************************************************************************************x*/
