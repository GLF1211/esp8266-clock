...
is a memory leak somewhere, crashes after decoding 1 fs::File :-)
...
#ifdef LOAD_SPIFFS
    if (jpg_source == JPEG_FS_FILE) g_pInFileFs.read(pBuf , n); // else we are handling a fs::File
#endif

#if defined (LOAD_SD_LIBRARY) || defined (LOAD_SDFAT_LIBRARY)
    if (jpg_source == JPEG_SD_FILE) g_pInFileSd.read(pBuf , n); // else we are handling a fs::File
#endif
...
// Generic fs::File call for SD or SPIFFS, uses leading / to distinguish SPIFFS files
...
int JPEGDecoder::decodeFsFile(fs::File jpgFile) { // This is for the SPIFFS library
    ...
    jpg_source = JPEG_FS_FILE; // Flag to indicate a SPIFFS fs::File

    if (!g_pInFileFs) {
#ifdef DEBUG
        Serial.println("ERROR: SPIFFS fs::File not found!");
#endif
...
int JPEGDecoder::decodeSdFile(const char* pFilename) {
    ...
    fs::File pInFile = SD.open(pFilename , FILE_READ);
...
int JPEGDecoder::decodeSdFile(const String& pFilename) {
    ...
    fs::File pInFile = SD.open(pFilename.c_str() , FILE_READ);
...
int JPEGDecoder::decodeSdFile(fs::File jpgFile) { // This is for the SD library

    g_pInFileSd = jpgFile;

    jpg_source = JPEG_SD_FILE; // Flag to indicate a SD fs::File

    if (!g_pInFileSd) {
#ifdef DEBUG
        Serial.println("ERROR: SD fs::File not found!");
#endif
...
int JPEGDecoder::decodeArray(const uint8_t array[] , uint32_t  array_size) {
    ...
    jpg_source = JPEG_ARRAY; // We are not processing a fs::File, use arrays
