...
// You will then need to directly reference the SPIFFS fs::File type thus in the sketch, e.g.:

fs::File jpegFile = SPIFFS.open( filename, "r"); // Example

// This will then allow the default method of using the SD library fs::File type to be used
// in the same sketch, e.g.:

fs::File jpegFile = SD.open( filename, FILE_READ);
