...
// A function called createArray() is provided to covert a jpeg fs::File stored on SD Card
// into a C array that can be pasted into any jpegX.h header fs::File. The array is sent
...
void drawSdJpeg(char* filename , int xpos , int ypos) {
    ...
    // Open the named fs::File (the Jpeg decoder library will close it)
    fs::File jpegFile = SD.open(filename , FILE_READ);  // or, fs::File handle reference for SD library

    if (!jpegFile) {
        Serial.print("ERROR: fs::File \""); Serial.print(filename); Serial.println("\" not found!");
        return;
    }

    Serial.println("===========================");
    Serial.print("Drawing fs::File: "); Serial.println(filename);
    Serial.println("===========================");

    // Use one of the following methods to initialise the decoder:
    boolean decoded = JpegDec.decodeSdFile(jpegFile);  // Pass the SD fs::File handle to the decoder,
...
void jpegRender(int xpos , int ypos) {
    ...
    //jpegInfo(); // Print information from the JPEG fs::File (could comment this line out)
    ...
    // Fetch data from the fs::File, decode and display
    while (JpegDec.read()) {    // While there is more data in the fs::File
...
void jpegInfo() {
    ...
    // Print information extracted from the JPEG fs::File
...
//####################################################################################################
// Pull a jpeg fs::File off the SD Card and send it as a "C" formatted as an array to the serial port
//####################################################################################################
...
void createArray(const char* filename) {
    ...
    // Check fs::File exists and open it
    if ((jpgFile = SD.open(filename)) == NULL) {
        Serial.println(F("JPEG fs::File not found"));
        return;
    }
    ...
    // Make the array the same as the fs::File name with the .(fs::File extension) removed
    while (*filename != '.') Serial.print(*filename++);

    Serial.println("[] PROGMEM = {"); // PROGMEM added for AVR processors, it is ignored by Due
    // Pull all data falues from fs::File and print in the array format
