/*
  ...
  Load the font fs::File onto the root directory of the SD Card.  The font files
  ...
  editting the User_Setup.h fs::File in the TFT_eSPI library folder.

  #########################################################################
  ###### DON'T FORGET TO UPDATE THE User_Setup.h fs::File IN THE LIBRARY ######
  #########################################################################
*/

// Font fs::File is stored on SD card
#include <SD.h>
...
void loop() {
    ...
    // Name of font fs::File (library adds leading / and .vlw)
...
void listDir(fs::FS& fs , const char* dirname , uint8_t levels) {
    ...
    while (fs::File) {
        if (fs::File.isDirectory()) {
            Serial.print("  DIR : ");
            Serial.println(fs::File.name());
            if (levels) {
                listDir(fs , fs::File.name() , levels - 1);
            }
        } else {
            Serial.print("  fs::File: ");
            Serial.print(fs::File.name());
            Serial.print("  SIZE: ");
            Serial.println(fs::File.size());
        }
