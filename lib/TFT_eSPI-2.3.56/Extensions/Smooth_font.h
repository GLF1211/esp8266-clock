...
// This is part of the TFT_eSPI class and is associated with anti-aliased font functions
 ...
 public:
     ...
     // These are for the metrics for each individual glyph (so we don't need to seek this in fs::File and waste time)
     ...
     uint32_t* gBitmap = NULL;   //fs::File pointer to greyscale bitmap
     ...
     fs::FS& fontFS = SPIFFS;
     bool     spiffs = true;
     bool     fs_font = false;    // For ESP32/8266 use smooth font fs::File or FLASH (PROGMEM) array
